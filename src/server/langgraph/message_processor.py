...
class LangGraphMessageProcessor:
        ...
        try:
            ...
            if result and "messages" in result and result["messages"]:
                ...
                if isinstance(ai_message, AIMessage):
                    ...
                    logger.info(f"Generated response for client {client_id}: {response_content[:50]}...")
                    return response_content
                else:
                    logger.warning(f"Last message is not an AI message for client {client_id}")
    ...
    async def handle_interrupts(self, client_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if current_state and current_state.next:
                ...
                logger.info(f"Client {client_id} has pending interrupt: {interrupt_info}")
    ...
    async def get_conversation_state(self, client_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error getting conversation state for client {client_id}: {str(e)}")
            return None
    ...
    async def resume_conversation(self, client_id: str, user_input: str) -> str:
        ...
        try:
            ...
            logger.error(f"Error resuming conversation for client {client_id}: {str(e)}")
            return "I encountered an error while processing your input. Please try again."
