...
class MessageProtocol:
    ...
    def parse_user_message(message_text: str) -> Optional[UserMessage]:
        ...
        try:
            ...
            logger.error(f"Failed to parse JSON message: {message_text[:100]}... Error: {str(e)}")
            return None
        except ValueError as e:
            logger.error(f"Invalid message data: {message_text[:100]}... Error: {str(e)}")
            return None
        except Exception as e:
            logger.error(f"Unexpected error parsing message: {message_text[:100]}... Error: {str(e)}")
            return None
    ...
    @staticmethod
    def create_error_message(error_code: str, message: str, retry_suggested: bool = True) -> str:
        """
