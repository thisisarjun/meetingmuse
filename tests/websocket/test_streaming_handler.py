...
from server.handlers.streaming_handler import StreamingHandler  # noqa: E402
...
class TestStreamingHandler:
    ...
    async def test_stream_response_success(self, mock_factory_class):
        ...
        assert any(
            update.get("type") in ["ai_response", "processing_step"]
    ...
    async def test_send_processing_notification(self, mock_factory_class):
        ...
        result = await handler.send_processing_notification(
            "intent_classification", "client123"
        )
    ...
    async def test_create_interrupt_notification(self, mock_factory_class):
        ...
        result = await handler.create_interrupt_notification(
            interrupt_info, "client123"
        )
