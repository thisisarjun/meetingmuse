...
class TestConversationManager:
    ...
    async def test_handle_reconnection_with_existing_conversation(
        ...
        mock_processor = Mock()
        mock_processor.get_conversation_state = AsyncMock(
            return_value={
    ...
    async def test_handle_reconnection_no_existing_conversation(
        ...
        mock_processor = Mock()
        mock_processor.get_conversation_state = AsyncMock(
            return_value={"has_conversation": False}
        )
        mock_processor_class.return_value = mock_processor
    ...
    @patch("meetingmuse_server.services.conversation_manager.LangGraphMessageProcessor")
    @pytest.mark.asyncio
    async def test_generate_conversation_summary_meeting_with_time(
        self, mock_processor_class
    ):
    ...
    async def test_update_conversation_activity(self, mock_processor_class):
        ...
        assert (
            manager.active_conversations["client123"]["message_count"]
            == initial_count + 1
        )
