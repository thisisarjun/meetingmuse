...
class TestGetNextNodeName(TestCollectingInfoNode):
    ...
    def test_get_next_node_name_with_meeting_details(self, node, meeting_details, expected_result, test_description):
        """
...
class TestIsMeetingDetailsComplete(TestCollectingInfoNode):
    ...
    def test_is_meeting_details_complete(self, node, meeting_details, expected_result, test_description):
        """
...
class TestUpdateStateMeetingDetails(TestCollectingInfoNode):
    ...
    def test_update_state_meeting_details(self, node, initial_state_details, new_meeting_details, expected_state_details, test_description):
        """
        ...
        state = MeetingMuseBotState(
            messages=[],
            meeting_details=initial_state_details
        )
        ...
        assert result_state.meeting_details.date_time == expected_state_details.date_time, f"Date time mismatch for case: {test_description}"
        assert result_state.meeting_details.participants == expected_state_details.participants, f"Participants mismatch for case: {test_description}"
        assert result_state.meeting_details.duration == expected_state_details.duration, f"Duration mismatch for case: {test_description}"
        assert result_state.meeting_details.location == expected_state_details.location, f"Location mismatch for case: {test_description}"
        
        # Verify the method returns the same state object (mutation)
        assert result_state is state, f"Method should return the same state object for case: {test_description}"
...
class TestInvokeExtractionPrompt(TestCollectingInfoNode):
    ...
    @pytest.mark.parametrize("meeting_details,missing_required,user_input,expected_result,test_description", [
        (
            MeetingFindings(
                participants=["john@example.com"],
            ),
    ...
    def test_invoke_extraction_prompt(self, node, meeting_details, missing_required, user_input, expected_result, test_description):
        """
        Test invoke_extraction_prompt returns correct meeting details based on user input.
        """
        # Act
        result = node.invoke_extraction_prompt(meeting_details, missing_required, user_input)
...
class TestInvokeMissingFieldsPrompt(TestCollectingInfoNode):
    ...
    def test_invoke_missing_fields_prompt(self, node, state, expected_result, test_description):
        """
