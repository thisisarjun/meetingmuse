...
class TestMeetingDetailsService:
    ...
    def test_is_meeting_details_complete(self, service, meeting_details, expected_result, test_description):
    ...
    @pytest.mark.parametrize("meeting_details,expected_missing,test_description", [
    ...
    def test_get_missing_required_fields(self, service, meeting_details, expected_missing, test_description):
        ...
        # Assert
        assert set(result) == set(expected_missing), f"Failed for case: {test_description}"
    ...
    def test_update_state_meeting_details(self, service):
        ...
        # Arrange
        initial_details = MeetingFindings(title="Old Meeting", date_time="2024-01-15 10:00 AM")
        state = MeetingMuseBotState(messages=[], meeting_details=initial_details)
        new_details = MeetingFindings(title="New Meeting", participants=["john@example.com"])
        ...
        assert result_state.meeting_details.date_time == "2024-01-15 10:00 AM"  # Should remain unchanged
