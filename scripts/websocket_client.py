    ...
    try:
        ...
        async with websockets.connect(uri) as websocket:
            ...
            try:
                ...
                if connection_msg.get("type") == "system" and connection_msg.get("content") == "connection_established":
            ...
            try:
                processing_response = await asyncio.wait_for(websocket.recv(), timeout=5.0)
                print(f"✓ Received processing message: {processing_response}")
                
                processing_msg = json.loads(processing_response)
                if processing_msg.get("type") == "system" and processing_msg.get("content") == "processing":
            ...
            try:
                ...
                if echo_msg.get("content") == expected_content:
                    ...
                    print(f"✗ Echo response doesn't match. Expected: {expected_content}, Got: {echo_msg.get('content')}")
            except asyncio.TimeoutError:
            ...
            try:
                ...
                if error_msg.get("type") == "error" and error_msg.get("error_code") == "INVALID_MESSAGE_FORMAT":
...
async def test_admin_endpoints():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post('http://localhost:8000/admin/broadcast', json=broadcast_data) as resp:
                if resp.status == 200:
                    data = await resp.json()
